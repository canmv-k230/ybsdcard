...
class CLPDPage(BaseDemoPage):
    ...
    def get_image_source(self):
        # 返回图片源
        try:
            with open("/sdcard/apps/ai_objects/car_license_plate_det/banner.png", 'rb') as f:
                img_cache = f.read()
                return lv.img_dsc_t({
    ...
    def get_title(self):
        return self.text_config.get_section("AI_objects")["car_license_plate_det"]["title"]
    
    def get_description(self):
        return self.text_config.get_section("AI_objects")["car_license_plate_det"]["info"]
    ...
    def on_button_click(self, evt):
        ...
        self.run_demo(car_license_plate_detection)