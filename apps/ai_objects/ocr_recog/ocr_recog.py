...
import apps.ai_objects.ocr_recog.ocr_recognition as ocr_recogection
from media.display import *
from media.media import *
import machine

class OCRRecogPage(BaseDemoPage):
    ...
    def get_image_source(self):
        # 返回图片源
        try:
            with open("/sdcard/apps/ai_objects/ocr_recog/banner.png", 'rb') as f:
                img_cache = f.read()
                return lv.img_dsc_t({
    ...
    def get_title(self):
        return self.text_config.get_section("AI_objects")["ocr_recog"]["title"]
    
    def get_description(self):
        return self.text_config.get_section("AI_objects")["ocr_recog"]["info"]
