...
import apps.ai_objects.ocr_det.ocr_detection as ocr_detection
from media.display import *
from media.media import *
import machine

class OCRDetPage(BaseDemoPage):
    ...
    def get_image_source(self):
        # 返回图片源
        try:
            with open("/sdcard/apps/ai_objects/ocr_det/banner.png", 'rb') as f:
                img_cache = f.read()
                return lv.img_dsc_t({
    ...
    def get_title(self):
        return self.text_config.get_section("AI_objects")["ocr_det"]["title"]
    
    def get_description(self):
        return self.text_config.get_section("AI_objects")["ocr_det"]["info"]
