        ...
        try:
            ...
            for det_box in det_boxes:
                try:
                    ...
                    print(f"Face pose estimation error: {e}")
                    continue  # 跳过处理失败的人脸，继续处理下一个
                    
            return det_boxes, pose_res
            
        except Exception as e:
            print(f"Face pose run error: {e}")
...
class YAHBOOM_DEMO:
    ...
    def exce_demo(self, loading_text="Loading ..."):
        ...
        self.fp=FacePose(face_det_kmodel_path,face_pose_kmodel_path,det_input_size=face_det_input_size,pose_input_size=face_pose_input_size,anchors=anchors,confidence_threshold=confidence_threshold,nms_threshold=nms_threshold,rgb888p_size=rgb888p_size,display_size=display_size)
        ...
        try:
            while True:
                ...
                det_boxes,pose_res=self.fp.run(img)          # 推理当前帧
                self.fp.draw_result(self.pl,det_boxes,pose_res)   # 绘制推理效果
                self.pl.show_image()                         # 展示推理效果
                gc.collect()
                time.sleep_us(1)
        except Exception as e:
            pass
