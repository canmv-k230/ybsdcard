...
import apps.ai_face.face_pose_core.face_pose as face_pose
from media.display import *
from media.media import *

class FacePosePage(BaseDemoPage):
    ...
    def get_image_source(self):
        # 返回图片源
        try:
            with open("/sdcard/apps/ai_face/face_pose_core/banner.png", 'rb') as f:
                img_cache = f.read()
                return lv.img_dsc_t({
    ...
    def get_title(self):
        return self.text_config.get_section("AI_face")["face_dirt_recog"]["title"]

    
    def get_description(self):
        return self.text_config.get_section("AI_face")["face_dirt_recog"]["info"]
