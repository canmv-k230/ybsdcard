...
# 导入所有页面
from apps.ai_face.face_recog_core.face_recog import FaceRecogPage
from apps.ai_face.face_det_core.face_det import FaceDetPage
from apps.ai_face.face_landmark_core.face_ldmark import FaceLandMarkPage
from apps.ai_face.face_pose_core.face_p import FacePosePage
from apps.ai_face.face_mesh_core.face_m import FaceMeshPage
from apps.ai_face.face_eye_pose_core.face_eye_p import FaceEyePosePage
from apps.ai_face.face_recog_register_core.face_recog_register import FaceRecogRegPage


class App(BaseApp):
    def __init__(self, app_manager):
        try:
            with open("/sdcard/apps/ai_face/icon.png", 'rb') as f:
            ...
            img_bg = None
        try:
            with open("/sdcard/apps/ai_face/dock_icon.png", 'rb') as f:
                bg_image_cache = f.read()
            ...
            print("open dock icon error", e)
            self.dock_icon = None 
                        
        self.config = app_manager.config
        self.text_config = app_manager.text_config
        
        super().__init__(app_manager, name=self.text_config.get_section("System")["AI_face"], icon=img_bg)
        
        pass
        
        # 类别和名称映射
        self.category_mapping = {
            self.text_config.get_section("AI_face")["face_det"]["title"]: "face_det",
            self.text_config.get_section("AI_face")["face_keypoint_recog"]["title"]: "face_ldmark",
            self.text_config.get_section("AI_face")["face_dirt_recog"]["title"]: "face_p",
            self.text_config.get_section("AI_face")["face_3d_net"]["title"]: "face_m",
            self.text_config.get_section("AI_face")["face_look_det"]["title"]: "face_eye_p",
            self.text_config.get_section("AI_face")["face_recog_register"]["title"]: "face_recog_register",
            self.text_config.get_section("AI_face")["face_recog"]["title"]: "face_recog"
    ...
    def _init_ai_face_pages(self):
        ...
        try:
            ...
            self.ai_face_pages["face_recog"] = FaceRecogPage(self, self.detail_panel, self.config, self.text_config)
            pass

            self.ai_face_pages["face_det"] = FaceDetPage(self, self.detail_panel, self.config, self.text_config)
            pass

            self.ai_face_pages["face_ldmark"] = FaceLandMarkPage(self, self.detail_panel, self.config, self.text_config)
            pass

            gc.collect()


            self.ai_face_pages["face_p"] = FacePosePage(self, self.detail_panel, self.config, self.text_config)
            pass

            self.ai_face_pages["face_m"] = FaceMeshPage(self, self.detail_panel, self.config, self.text_config)
            pass

            self.ai_face_pages["face_eye_p"] = FaceEyePosePage(self, self.detail_panel, self.config, self.text_config)
            pass

            gc.collect()
            
            self.ai_face_pages["face_recog_register"] = FaceRecogRegPage(self, self.detail_panel, self.config, self.text_config)
            pass
    ...
    def _select_category(self, category, btn):
        ...
        if page_key and page_key in self.ai_face_pages:
            page = self.ai_face_pages[page_key]
            page.display()
        else:
            pass
    ...
    def deinitialize(self):
        ...
        pass
        # 清理所有AI人脸页面
        for page_key, page in self.ai_face_pages.items():
