...
import apps.ai_face.face_mesh_core.face_mesh as face_mesh
from media.display import *
from media.media import *

class FaceMeshPage(BaseDemoPage):
    ...
    def get_image_source(self):
        # 返回图片源
        try:
            with open("/sdcard/apps/ai_face/face_mesh_core/banner.png", 'rb') as f:
                img_cache = f.read()
                return lv.img_dsc_t({
    ...
    def get_title(self):
        return self.text_config.get_section("AI_face")["face_3d_net"]["title"]

    
    def get_description(self):
        return self.text_config.get_section("AI_face")["face_3d_net"]["info"]
