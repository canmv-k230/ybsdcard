...
class EyeGaze:
    ...
    def run(self, input_np):
        ...
        返回/Returns:
            ...
            eye_gaze_res: 注视估计结果/Gaze estimation results
        '''
        try:
            # 先进行人脸检测
            ...
            for det_box in det_boxes:
                try:
                    ...
                    print(f"Eye gaze estimation error: {e}")
                    continue  # 跳过处理失败的人脸，继续处理下一个
                    
            return det_boxes, eye_gaze_res
            
        except Exception as e:
            print(f"Eye gaze run error: {e}")
 ...
 size = 30, thickness = 4)
                uart.send(f"${int(center_x)},{int(center_y)},{target_x},{target_y}#")
...
# 执行演示的主函数 / Main function to execute demonstration
    def exce_demo(self, loading_text="Loading ..."):
        ...
        self.eg=EyeGaze(face_det_kmodel_path,eye_gaze_kmodel_path,det_input_size=face_det_input_size,eye_gaze_input_size=eye_gaze_input_size,anchors=anchors,confidence_threshold=confidence_threshold,nms_threshold=nms_threshold,rgb888p_size=rgb888p_size,display_size=display_size)
                ...
                if len(point):
                    ...
                    if pt.event == TOUCH.EVENT_DOWN:
                        if pt.x<100 and pt.y<100:
                            ...
                            time.sleep_ms(10)
                            break
                img=self.pl.get_frame()                          # Get the current frame
                det_boxes,eye_gaze_res=self.eg.run(img)          # Run inference on the current frame
                self.eg.draw_result(self.pl,det_boxes,eye_gaze_res)   # Draw the inference result
                self.pl.show_image()                             # Display the result
                gc.collect()
                time.sleep_us(1)
        except Exception as e:
            pass
