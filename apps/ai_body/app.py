...
# 导入所有页面
from apps.ai_body.person_det_core.person_det import PersonDetPage
from apps.ai_body.person_keypoint_det_core.person_keypoint_det import PersonKeypointDetPage
from apps.ai_body.person_falldown_det_core.person_falldown_det import PersonFalldownDetPage
from apps.ai_body.person_hand_det_core.person_hand_det import PersonHandDetPage
from apps.ai_body.person_hand_keypoint_det_core.person_hand_keypoint_det import PersonHandKeypointDetPage
from apps.ai_body.person_hand_keypoint_class_core.person_hand_keypoint_cla import PersonHandKeypointClaPage
from apps.ai_body.person_hand_recognition_core.person_hand_recog import PersonHandRecogPage
# from apps.ai_body.person_finger_guessing_core.person_finger_guessing import PersonFingerGuessingPage


class App(BaseApp):
    def __init__(self, app_manager):
        try:
            with open("/sdcard/apps/ai_body/icon.png", 'rb') as f:
        ...
        self.menu_order = [
            (self.text_config.get_section("AI_body")["person_det"]["title"], "person_det"),
            (self.text_config.get_section("AI_body")["person_keypoint_det"]["title"], "person_keypoint_det"),
            (self.text_config.get_section("AI_body")["person_falldown_det"]["title"], "person_falldown_det"),
            (self.text_config.get_section("AI_body")["person_hand_det"]["title"], "person_hand_det"),
            (self.text_config.get_section("AI_body")["person_hand_keypoint_det"]["title"], "person_hand_keypoint_det"),
            (self.text_config.get_section("AI_body")["person_hand_keypoint_cla"]["title"], "person_hand_keypoint_cla"),
            # (self.text_config.get_section("AI_body")["person_hand_recog"]["title"], "person_hand_recog"),
        ]
        super().__init__(app_manager, name=self.text_config.get_section("System")["AI_body"], icon=img_bg)
        pass
    ...
    def _init(self):
        try:
            ...
            self._init_ai_body_pages()
        
            # 默认选择第一个类别
            first_category, _ = self.menu_order[0]  # 从menu_order获取第一个类别名称
            # 获取第一个按钮
            if self.category_list and self.category_list.get_child_cnt() > 0:
                first_btn = self.category_list.get_child(0)
                print(first_category, first_btn)
    ...
    def _init_ai_body_pages(self):
        ...
        try:
            ...
            self.ai_body_pages = {}
            self.ai_body_pages["person_det"] = PersonDetPage(self, self.detail_panel, self.config, text_config)
            pass
            
            self.ai_body_pages["person_keypoint_det"] = PersonKeypointDetPage(self, self.detail_panel, self.config, text_config)
            pass
            
            self.ai_body_pages["person_falldown_det"] = PersonFalldownDetPage(self, self.detail_panel, self.config, text_config)
            pass
            
            self.ai_body_pages["person_hand_det"] = PersonHandDetPage(self, self.detail_panel, self.config, text_config)
            pass
            
            self.ai_body_pages["person_hand_keypoint_det"] = PersonHandKeypointDetPage(self, self.detail_panel, self.config, text_config)
            pass
            
            self.ai_body_pages["person_hand_keypoint_cla"] = PersonHandKeypointClaPage(self, self.detail_panel, self.config, text_config)
            pass
            
            # self.ai_body_pages["person_finger_guessing"] = PersonFingerGuessingPage(self, self.detail_panel, self.config)
            # pass
            
            # self.ai_body_pages["person_hand_recog"] = PersonHandRecogPage(self, self.detail_panel, self.config, text_config)
            pass
    ...
    def _create_split_view(self):
        ...
        for category, _ in self.menu_order:  # 使用menu_order而不是category_mapping.keys()
    ...
    def _select_category(self, category, btn):
        ...
        if page_key and page_key in self.ai_body_pages:
            page = self.ai_body_pages[page_key]
            page.display()
        else:
            pass
    ...
    def deinitialize(self):
        ...
        pass
        # 清理所有AI人脸页面
        for page_key, page in self.ai_body_pages.items():
